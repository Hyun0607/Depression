...
- 다양한 전처리 및 분석 기법을 실습 환경에서 직접 구현
- 변수 라벨링, 범주화, 샘플링 전략이 예측 모델에 미치는 영향을 실험적으로 검토
- 로지스틱 회귀, 의사결정나무, 랜덤포레스트 간 성능·복잡도·해석력을 비교하며 균형 평가
- 변수 중요도 분석을 통해 예측에 핵심적으로 작용한 변수 도출 및 해석
  - 예: `Number of Children`, `Alcohol Consumption`, `History of Substance Abuse` 등은 높은 중요도를 보여 우울증 예측에 크게 기여함
  - 반대로 `Marital Status`, `Income`, `Dietary Habits` 등은 상대적으로 낮은 기여도를 보임
- 변수 중요도 시각화를 통해 **개인의 건강 상태 및 생활습관이 우울증 예측에 영향을 미침**을 시각적으로 확인
